...
function registerSettings(){
  ...
  register(SETTINGS.SCHEDULE_STATE,{scope:"client",config:false,type:Object,default:{}});
...
function addTokenHudButton(app,html,data){
  if(!game.user?.isGM)return;const root=html instanceof HTMLElement?html:html?.[0];if(!root)return;const token=app.object??canvas?.tokens?.get?.(data?._id),document=token?.document??canvas?.scene?.tokens?.get?.(data?._id);if(!document)return;const flag=repository.getOtherworldly(document),column=root.querySelector(".col.right")??root.querySelector(".right")??root;const button=globalThis.document.createElement("div");button.className=`control-icon ov-token-hud ${flag.enabled?"active":""}`;button.title=game.i18n.localize(flag.enabled?"OV.TokenHud.Enabled":"OV.TokenHud.Disabled");button.setAttribute("role","button");button.setAttribute("tabindex","0");button.setAttribute("aria-label",button.title);button.innerHTML=`<i class="fa-solid ${flag.enabled?"fa-eye":"fa-eye-slash"}" aria-hidden="true"></i>`;let toggleBusy=false;const toggle=async event=>{event.preventDefault();event.stopPropagation();if(toggleBusy)return;toggleBusy=true;button.dataset.busy="true";button.setAttribute("aria-busy","true");try{try{const enabled=repository.getOtherworldly(document).enabled;await repository.setOtherworldly(document,{enabled:!enabled});}catch(error){warn("Token HUD toggle failed",error);ui.notifications?.error?.(game.i18n.format("OV.Notifications.ActionFailed",{action:button.title,error:error?.message??String(error)}));return;}try{await app.render(true);}catch(error){warn("Token HUD render failed",error);}}finally{toggleBusy=false;delete button.dataset.busy;button.removeAttribute("aria-busy");}};button.addEventListener("click",event=>{void toggle(event);});button.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" ")void toggle(event);});button.addEventListener("contextmenu",event=>{event.preventDefault();openTokenEditor(document);});column.appendChild(button);
}
