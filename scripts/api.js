...
export function createApi(){
  const api={
    ...
    async setTouched(actor,patch){requireGM("change Touched actor data");return clone(await repository.setTouched(actor,patch));},
    async setRevelation(actor,tokenUuid,stage){requireGM("change revelation progress");return clone(await repository.setRevelation(actor,tokenUuid,stage));},
    async setOtherworldly(token,patch){requireGM("change Otherworldly token data");return clone(await repository.setOtherworldly(token,patch));},
    async upsertSet(set,options={}){requireGM("change vision sets");return clone(await repository.upsertSet(set,options));},
    async deleteSet(uuid,options={}){requireGM("delete vision sets");return clone(await repository.deleteSet(uuid,options));},
