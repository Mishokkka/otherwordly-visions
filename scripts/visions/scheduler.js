...
export class VisionScheduler {
  ...
  reconcile(){
    ...
    this.running=true; for(const set of sets) this.schedule(set,generation,false); this.emit();
  }
  ...
  eligibleSets(){
    const user=game.user;if(!user||user.isGM)return[];const actors=[];if(user.character)actors.push(user.character);for(const actor of game.actors??[])if((actor.testUserPermission?.(user,"OWNER")??false)&&!actors.some(item=>item.id===actor.id))actors.push(actor);
    ...
    return [...assigned].map(uuid=>repository.getSet(uuid)).filter(set=>set?.enabled&&(set.images.length||set.audio.length||set.entries.length||set.sequence.length||set.playlistIds.length));
  }
  schedule(setOrUuid,generation,emit=true){
    ...
    const delayMs=randomBetween(set.minDelay,set.maxDelay)*1000;const nextAt=Date.now()+delayMs;const timer=window.setTimeout(()=>void this.run(uuid,generation),delayMs);this.jobs.set(uuid,{setUuid:uuid,generation,timer,nextAt,running:false});this.prewarm(set);if(emit)this.emit();
  }
  async run(uuid,generation){
    ...
    finally{if(this.running&&generation===this.generation&&this.eligibleUuids.has(uuid))this.schedule(uuid,generation,false);this.emit();}
  }
