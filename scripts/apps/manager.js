...
export function makeManagerClass(){
  ...
  return class OtherworldlyManager extends HandlebarsApplicationMixin(ApplicationV2){
    ...
    static async scanAssets(){if(!this.draft)return;this.syncDraft();this.scanResults=[];this.scanProgress={completed:0,total:0};this.render();this.scanResults=await mediaCache.scanSet(this.draft,progress=>{this.scanProgress=progress;const bar=this.element?.querySelector?.("[data-scan-progress]");if(bar){bar.max=Math.max(1,Number(progress.total)||1);bar.value=Number(progress.completed)||0;bar.setAttribute("aria-valuetext",`${progress.completed ?? 0} / ${progress.total ?? 0}`);}});this.scanProgress=null;this.render();}
    ...
    static async restoreMigrationBackup(){if(!await this.confirmDiscardDraft())return;const yes=await confirmDialog(game.i18n.localize("OV.Confirm.RestoreTitle"),game.i18n.localize("OV.Confirm.RestoreBody"));if(!yes)return;await repository.restoreMigrationBackup();this.draft=null;this.original=null;location.reload();}
